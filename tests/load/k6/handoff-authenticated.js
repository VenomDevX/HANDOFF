...
export function setup() {
  if (!EMAIL || !PASSWORD) {
    console.error('LOAD_TEST_EMAIL and LOAD_TEST_PASSWORD are required');
...
export default function runTest(data) {
  ...
  const routes = [
    { path: '/dashboard', name: 'dashboard SSR' },
    { path: '/api/v1/dashboard/overview', name: 'api dashboard overview' },
    { path: '/api/v1/tasks', name: 'api tasks' },
    { path: '/api/v1/projects', name: 'api projects' },
    { path: '/api/v1/notifications', name: 'api notifications' }
  ];
